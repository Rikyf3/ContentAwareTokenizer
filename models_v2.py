...
from layers import MemEffTokenCreation
...
class vit_models(nn.Module):
    ...
    def __init__(self, img_size=224,  patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 Attention_block = Attention, Mlp_block=Mlp, Token_creation=None,
        ...
        self.token_creation = Token_creation() if Token_creation is not None else nn.Identity()
    ...
    def no_weight_decay(self):
        s = {'pos_embed', 'cls_token'}
        # s.update({name for name, _ in self.named_parameters() if any(x in name for x in ['constant_attn', 'cross_attention.weight'])})
        # print(s)
        return s

    @torch.jit.ignore
    def get_params_groups(self, args):
        param_groups = []
        ...
        for name, param in self.named_parameters():
            ...
            d = {"params" : param, "weight_decay" : args.weight_decay, "lr" : args.lr}

            if any(x in name for x in ["pos_embed", "cls_token", "token_creation", "norm", "bias"]):
                #print(name)
                d.update({"weight_decay" : 0.0})

            param_groups.append(d)
    ...
    def interpolate_pos_encoding(self, x, w, h):
        ...
        patch_pos_embed = nn.functional.interpolate(
            self.pos_embed.reshape(1, int(sqrt_N), int(sqrt_N), dim).permute(0, 3, 1, 2),
            scale_factor=(sx, sy),
            mode="bicubic",
        )
        ...
        return patch_pos_embed.permute(0, 2, 3, 1).view(1, -1, dim).to(previous_dtype)

    def forward_features(self, x):
        B, _, w, h = x.shape
        x = self.patch_embed(x)
        #x = self.token_creation(x)
...
@register_model
def deit_small_patch16_LS_Dynamic(pretrained=False, img_size=224, pretrained_21k = False,  **kwargs):
    model = vit_models(
        img_size = img_size, patch_size=4, embed_dim=384, depth=12, num_heads=6, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6),block_layers=Layer_scale_init_Block, Token_creation=partial(MemEffTokenCreation, embed_dim=384, num_tokens=196), **kwargs)
